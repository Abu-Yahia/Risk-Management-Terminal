...
def generate_risk_details(subject):
    ...
    Categories allowed: {", ".join(MAIN_CATEGORIES)}.
    ...
    try:
        ...
        # استخراج النص وتنظيفه من أي علامات Markdown
...
# --- 5. واجهة المستخدم ---
st.title("🛡️ SEF Risk Intelligence (Gemini Free Edition)")
st.markdown("---")
...
if 'current_risk' in st.session_state:
    ...
    st.divider()
    csv_data = pd.DataFrame([data]).to_csv(index=False).encode('utf-8-sig')
    st.download_button("📥 Download CSV", csv_data, f"Risk_{data.get('Risk ID','ID')}.csv")
